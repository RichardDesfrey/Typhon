...
; Демонология
       ...
       call vFree
; Блок
       stdcall LoadFile, Block_mop
       push eax
       stdcall MemCopy, eax, Block_Table, 1000
       pop ecx
       call vFree
...
proc Properties3
       mov [CasterAngDem], eax
       movsx edx, byte [eax+Properties3_Table]
       cmp edx, 9;  Оживление Мертвецов
       je AngDem3; проверки на Демонологию, но без них - прыжок на обычное Всокрешение
       jmp dword [4476FCh+edx*4]
endp

proc Animate_Dead1; ИИ
       mov edx, [ebx+84h]; затёртый код, получить флаги отряда
       mov eax, [CasterAngDem]
       cmp byte [eax+Properties3_Table], 9; 9 - Оживление Мертвецов
       je @f
       push 5A40C7h; стандартная проверка флагов живого
       ret
     @@:
       shr edx, 12h; флаг Нежити
       push 5A40CAh
       ret
endp

proc Animate_Dead2; Человек
       mov edx, [esi+84h]; затёртый код, получить флаги отряда
       mov ecx, [CasterAngDem]
       cmp byte [ecx+Properties3_Table], 9; 9 - Оживление Мертвецов
       je @f
       push 5A402Fh; стандартная проверка флагов живого
       ret
     @@:
       shr edx, 12h; флаг Нежити
       push 5A4032h
...
proc Animate_Dead3; Человек
       mov eax, [edx+84h]; затёртый код, получить флаги отряда
       mov ecx, [CasterAngDem]
       cmp byte [ecx+Properties3_Table], 9; 9 - Оживление Мертвецов
       je @f
       push 44092Ah; стандартная проверка флагов живого
       ret
     @@:
       shr eax, 12h; флаг Нежити
       push 44092Dh
       ret
endp

proc Animate_Dead4; Общая
       mov ecx, [CasterAngDem]
       or [CasterAngDem], -1
       test ecx, ecx
       jl .not_caster
...
proc Ang1
       mov [CasterAngDem], eax; запоминаем для последующего использования в расчете демонологии (proc Demon2)
       cmp byte [eax+Properties5_Table], 0
       jnz @f
       push 44705Fh
       ret
...
proc AngDem2
       mov [CasterAngDem], eax; запоминаем для последующего использования в расчете демонологии (proc Demon)
       cmp byte [eax+Properties5_Table], 0
       je .ret
       cmp byte [eax+Properties5_Table], 2
       jnz .no
...
proc AngDem4
       cmp byte [ecx+Properties5_Table], 0
       je .ret
       cmp byte [ecx+Properties5_Table], 2
       jnz .no
     .ret:
...
proc Demon
       mov edi, [CasterAngDem]; получить монстра-колдуна
...
proc Block
       mov eax, [ecx+34h]
       movsx edx, byte [eax+Block_Table]
       push 766A9Bh; push    edx
       ret
endp